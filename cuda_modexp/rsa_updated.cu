...
#define MAX_DIGITS 4
// static _device_ void print_limbs(const char* label, const uint64_t* limbs, int digits) {
//     printf("%s: ", label);
...
printf("%016lx", d_mu[0]);  // prints the first uint64_t value

     printf("After\n");
    printf("\n");

    // Copy cipher back to host
// cudaError_t err = cudaMemcpy(cipher, d_cipher, MAX_DIGITS * sizeof(uint64_t), cudaMemcpyDeviceToHost);
